...
def process_file(filename, keyfiles):
    ...
    if len(header) != 512:
        f.close()
        sys.stderr.write("%s : Truecrypt volume file too short: Need at least 512 bytes\n" % filename)
        return

    for tag in ["truecrypt_RIPEMD_160", "truecrypt_SHA_512", "truecrypt_WHIRLPOOL"]:
        sys.stdout.write("%s:%s$" % (basename(filename), tag))
        sys.stdout.write(binascii.hexlify(header).decode())  # Convert bytes to str using decode()
        if keyfiles:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        sys.stderr.write("\nUtility to import TrueCrypt volume to a format crackable by John The Ripper\n")
